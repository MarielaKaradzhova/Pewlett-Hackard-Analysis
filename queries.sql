...
-- Number of employees retiring with specific birth dates
...
-- Joins
-- Join the departments table with the dept_manager table
...
-- Create a new table for retiring employees after dropping retirement_info 
...
--Recreate the retirement_info Table with the dept_emp table
...
-- Employee count by department number with new table emp_count
SELECT COUNT(ce.emp_no), de.dept_no
INTO emp_count
FROM current_emp as ce
LEFT JOIN dept_emp as de
...
--sort that column in descending order and filter employment dates like employees table
